<template>
  <div ref="calendar" class="mpvue-calendar">
    <div v-if="!isMonthRange" class="calendar-tools">
      <div class="calendar-prev" @click="prev">
        <img v-if="!!arrowLeft" :src="arrowLeft" />
        <i v-else class="iconfont icon-arrow-left"></i>
      </div>
      <div class="calendar-next" @click="next">
        <img v-if="!!arrowRight" :src="arrowRight" />
        <i v-else class="iconfont icon-arrow-right"></i>
      </div>
      <div class="calendar-info" @click.stop="changeYear">
        <div class="mc-month">
          <div v-if="isIos" :class="['mc-month-inner', oversliding ? '' : 'month-transition']">
            <span>{{ monthsLoop[monthIndex] }}</span>
          </div>
          <div v-else class="mc-month-text">{{ monthText }}</div>
        </div>
    ...
    <table cellpadding="5">
      ...
      <div
        v-for="(itemDays, index) in monthRangeDays"
        :key="index"
        :class="[
          'mc-body',
          { 'mc-range-mode': range, 'week-switch': weekSwitch && !isMonthRange, 'month-range-mode': isMonthRange }
        ]"
      >
        <div v-if="isMonthRange" class="month-rang-head">{{ rangeOfMonths[index][2] }}</div>
        <tr v-for="(itemDay, k1) in itemDays" :key="k1" :class="{ gregorianStyle: !lunar }">
          <td
            v-for="(child, k2) in itemDay"
            :key="k2"
            :class="[
              {
                ...
                'mc-range-select-one': rangeBgHide && child.selected,
                lunarStyle: lunar,
          ...
          >
            ...
            <span v-else :class="[{ 'mc-date-red': k2 === (monFirst ? 5 : 0) || k2 === 6 }, 'calendar-date']">{{
              child.day
            }}</span>
            <div v-if="!!child.content" class="slot-element">{{ child.content }}</div>
            <div v-if="child.eventName && !clean" class="mc-text remark-text">{{ child.eventName }}</div>
            <div v-if="child.eventName && clean" class="mc-dot" />
            <div
            ...
            <div v-if="range && child.selected" class="mc-range-bg" />
          </td>
        </tr>
      </div>
    </table>
    <div class="mpvue-calendar-change" :class="{ show: yearsShow }">
      <div v-if="!weekSwitch" class="calendar-years">
      ...
      <div :class="['calendar-months', { 'calendar-week-switch-months': weekSwitch }]">
        <span v-for="(m, i) in months" :key="m" :class="{ active: i === month }" @click.stop="changeMonth(i)">{{
          m
        }}</span>
      </div>
    </div>
