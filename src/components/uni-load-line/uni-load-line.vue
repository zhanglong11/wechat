<template>
  <view>
    <view>
      <view v-if="loadType === 'line'" class="line">
        <view
          class="loadline"
          :style="{
            width: loadPercent + '%',
            background: lineInfo.colorChange
              ? 'linear-gradient(to right,' + lineInfo.loadColor + ')'
              : lineInfo.loadColor.split(',')[0]
          }"
        ></view>
        <view class="v-title">
      ...
      <view v-if="loadType === 'circle'" class="circle">
        <view class="third" :style="{ background: circleInfo.background }">
          <view v-if="loadPercent !== 100" class="second" :style="{ background: circleInfo.foreground }"></view>
          <view
            class="second_"
            :style="{
              transform: 'rotate(' + (loadPercent >= 50 ? ((loadPercent - 50) / 100) * 360 : 0) + 'deg)',
              background: circleInfo.background
            }"
          ></view>
          <view
            v-if="loadPercent < 50"
            class="first"
            :style="{ transform: 'rotate(' + 360 * (loadPercent / 100) + 'deg)', background: circleInfo.foreground }"
          ></view>
      ...
      <view
        v-if="loadType === 'ball'"
        class="ball"
        :style="{ width: ballSize_ + 25 + 'px', height: ballSize_ + 25 + 'px' }"
      >
        <view
          class="loading-ball"
          :style="{ width: ballSize_ + 'px', height: ballSize_ + 'px', padding: ballInfo.ballPadding }"
        >
          <view
            class="loading-wave"
            :style="{
              overflow: ballInfo.ballType === 'all' ? 'initial' : 'hidden',
              width: ballSize_ + 'px',
              height: ballSize_ + 'px'
            }"
          >
            <view
              ...
              class="loading-wave1"
              :style="{
                background: ballInfo.waveColor,
                top: (ballInfo.ballType === 'all' ? -(loadPercent * 1.3) : -(loadPercent * 1.2)) - 4 + '%',
                width: ballSize_ * 2 + 'px',
                height: ballSize_ * 2 + 'px'
              }"
            ></view>
            <view
              v-if="loadPercent < 100"
