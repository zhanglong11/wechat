...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  var seriesConvert = series.map(function(item) {
    ...
    return {
      ...
      noneLinePosition: item.noneLinePosition
    }
  })
  for (let i = 0; i < seriesConvert.length; i++) {
    ...
    if (item.noneLinePosition) {
      orginX1 = Math.cos(item.arc) * (item.radius - 40)
      orginY1 = Math.sin(item.arc) * (item.radius - 20)
    }
...
function drawPieDataPoints(series, opts, config, context) {
  ...
  // 自适应的radius不建议使用；使用opts.radius，自定义大于0的值即可
  if (opts.radius) {
    series = getPieDataPoints(series, opts.radius, process)
  } else {
